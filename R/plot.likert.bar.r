...
likert.bar.plot <- function(l,
    ...
    if(!is.null(l$grouping)) { ##### Grouping ##################################
        ...
        if(plot.percent.low) {
            p <- p + ggrepel::geom_text_repel(direction = 'y',data=lsum, y=ymin, aes(x=Group, 
        ...
        if(plot.percent.high) {
            p <- p + ggrepel::geom_text_repel(direction = 'y',data=lsum, aes(x=Group, y=100, 
        ...
        if(plot.percent.neutral & l$nlevels %% 2 == 1 & include.center) {
            if(centered) {
                p <- p + ggrepel::geom_text_repel(direction = 'y',data=lsum, y=0, aes(x=Group, group=Item,
                ...
                p <- p + ggrepel::geom_text_repel(direction = 'y',data=lsum, aes(x=Group, y=y, group=Item,
                                   label=paste0(prettyNum(neutral, digits=digits, drop0trailing=drop0trailing, zero.print=zero.print), '%')),
                                   size=text.size, hjust=.5, color=text.color)              
            }
        }
        if(plot.percents) {
            ...
            p <- p + ggrepel::geom_text_repel(direction = 'y',data = lpercentpos, 
                               aes(x = Group, y = pos, 
            ...
            if(nrow(lpercentneg) > 0) {
                ...
                p <- p + ggrepel::geom_text_repel(direction = 'y',data = lpercentneg,
                                   aes(x = Group, y = pos, 
        ...
        if(plot.percent.low) {
            p <- p + ggrepel::geom_text_repel(direction = 'y',data=lsum, y=ymin, aes(x=Item, 
                                label=paste0(prettyNum(low, digits=digits, drop0trailing=drop0trailing, zero.print=zero.print), '%')), 
                                size=text.size, hjust=1, color=text.color)
        }
        if(plot.percent.high) {
            p <- p + ggrepel::geom_text_repel(direction = 'y',data=lsum, y=100, aes(x=Item,
                            label=paste0(prettyNum(high, digits=digits, drop0trailing=drop0trailing, zero.print=zero.print), '%')), 
        ...
        if(plot.percent.neutral & l$nlevels %% 2 == 1 & include.center & !plot.percents) {
            if(centered) {
                p <- p +
                    ggrepel::geom_text_repel(direction = 'y',data=lsum, y=0, 
                              aes(x=Item, 
                              label=paste0(prettyNum(neutral, digits=digits, drop0trailing=drop0trailing, zero.print=zero.print), '%')),
                              size=text.size, hjust=.5, color=text.color)
            } else {
                lsum$y <- lsum$low + (lsum$neutral/2)
                p <- p +
                    ggrepel::geom_text_repel(direction = 'y',data=lsum,
                              aes(x=Item, y=y, 
                              label=paste0(prettyNum(neutral, digits=digits, drop0trailing=drop0trailing, zero.print=zero.print), '%')),
                              size=text.size, hjust=.5, color=text.color)               
            }
        ...
        if(plot.percents) {
            ...
            lpercentpos <- ddply(results[results$value > 0,], .(Item), transform, 
                                 pos = cumsum(value) - 0.5*value)
            p <- p + ggrepel::geom_text_repel(direction = 'y',data=lpercentpos[lpercentpos$variable != center.label,], 
                               aes(x=Item, y=pos, 
                        label=paste0(prettyNum(value, digits=digits, drop0trailing=drop0trailing, zero.print=zero.print), '%')),
            ...
            if(nrow(lpercentneg) > 0) {
                ...
                lpercentneg <- ddply(lpercentneg, .(Item), transform, 
                                     pos = cumsum(value) - 0.5*value)   
                lpercentneg$pos <- lpercentneg$pos * -1
                p <- p + ggrepel::geom_text_repel(direction = 'y',data=lpercentneg[lpercentneg$variable != center.label,], 
                                   aes(x=Item, y=pos, 
                            label=paste0(prettyNum(abs(value), digits=digits, drop0trailing=drop0trailing, zero.print=zero.print), '%')),
                            size=text.size, color=text.color)
            }
